...
public class Test20210907 {
	 ...
	 * 문제
	 	...
	 	민우의 동생이 로또에 낙서를 하여, 일부 번호를 알아볼 수 없게 되었습니다. 당첨 번호 발표 후, 민우는 자신이 구매했던 로또로 당첨이 가능했던 최고 순위와 최저 순위를 알아보고 싶어 졌습니다.
...
//		[45, 4, 35, 20, 3, 9]	[20, 9, 3, 45, 4, 35]	[1, 1]
		...
		int[] result = solution1(lottos, win_nums);
		
		System.out.println(Arrays.toString(result));
		
		result = solution2(lottos, win_nums);
		System.out.println(Arrays.toString(result));
	}
	...
	public static int[] solution2(int[] lottos, int[] win_nums) {
		...
		for( int win_num : win_nums ) {
			boolean isExist = map.containsKey( win_num );
			if( isExist ) same++; 
		}
	...
	public static int[] solution1(int[] lottos, int[] win_nums) {
        ...
        for( int i = 0; i < win_nums.length; i++ ) {
        	int index = Arrays.binarySearch( lottos, win_nums[i] );
        	if( lottos[i] == 0 ) zero++;
        	if( index > -1 ) cnt++;
        }
